...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="h-16 md:h-18 bg-white dark:bg-slate-900 border-b dark:border-slate-800 grid grid-cols-3 items-center px-3 md:px-6 z-50 shrink-0 shadow-xs relative">
      {/* Logos Left */}
      <div className="flex items-center space-x-2 md:space-x-2.5 text-left">
        <img
          ...
          onClick={onGoHome}
          className="h-8 md:h-10 w-auto object-contain cursor-pointer hover:scale-105 transition"
        />
      ...
      <div className="flex flex-col items-center justify-center">
        <span className="font-black text-blue-900 dark:text-blue-400 uppercase text-[10px] sm:text-sm md:text-lg lg:text-xl tracking-tighter whitespace-nowrap">
          Vigilância Sanitária e Ambiental
        </span>
        <div className="flex items-center gap-1.5 mt-0.5">
          <button
            onClick={onToggleDarkMode}
            className="text-[8px] font-black bg-slate-100 dark:bg-slate-800 px-2.5 py-0.5 rounded-full border dark:border-slate-700 uppercase hover:bg-slate-200 dark:hover:bg-slate-700 transition cursor-pointer"
          >
            Tema 🌓
          </button>
        </div>
      ...
      <div className="flex flex-col items-end leading-tight text-right relative">
        <div
          ...
          className="cursor-pointer group bg-slate-50 dark:bg-slate-800/80 hover:bg-blue-50/80 dark:hover:bg-slate-800 px-2.5 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 transition flex items-center gap-2 shadow-2xs hover:shadow-xs"
        ...
        >
          ...
          <div className="w-7 h-7 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-black text-xs flex items-center justify-center uppercase shadow-xs group-hover:scale-105 transition shrink-0">
        ...
        {dropdownOpen && (
          <div className="absolute right-0 top-12 w-64 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl z-[1000] p-2.5 text-left space-y-2">
